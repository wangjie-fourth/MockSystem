...
import org.github.fourth.mocksystem.packages.mockit.coverage.paths.NodeBuilder;
...
import static org.github.fourth.mocksystem.packages.mockit.coverage.Metrics.DataCoverage;
import static org.github.fourth.mocksystem.packages.mockit.coverage.Metrics.PathCoverage;
...
final class CoverageModifier extends ClassVisitor {
    ...
    private static String getKindOfJavaType(int typeModifiers, @Nonnull String superName) {
        if ((typeModifiers & ACC_ANNOTATION) != 0) {
            return "annotation";
        } else if ((typeModifiers & ACC_INTERFACE) != 0) {
            return "interface";
        } else if ((typeModifiers & ACC_ENUM) != 0) {
            return "enum";
        } else if ((typeModifiers & ACC_ABSTRACT) != 0) {
            return "abstractClass";
        } else if (superName.endsWith("Exception") || superName.endsWith("Error")) {
            return "exception";
        }
        return "class";
    }
    ...
    private final class MethodModifier extends MethodOrConstructorModifier {
        MethodModifier(@Nonnull MethodWriter mw) {
            super(mw);
        }
    ...
    private final class ConstructorModifier extends MethodOrConstructorModifier {
        ConstructorModifier(@Nonnull MethodWriter mw) {
            super(mw);
        }
    }

    private final class StaticBlockModifier extends BaseMethodModifier {
        StaticBlockModifier(@Nonnull MethodWriter mw) {
            super(mw);
        }
